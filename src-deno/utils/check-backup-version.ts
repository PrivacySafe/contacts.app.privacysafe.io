...
import type { BackupVersionWarning } from '../../src/types/index.ts';
 ...
 * format and the contacts table schema are what break, and they do not change
 * with every minor release: gating on the app version instead would turn every
 * archive taken before 0.9.0 into a red "restore at own risk" the day 0.9.0
...
export function checkBackupVersionCompatibility(
  ...
  const isCompatible = (appParts[0] === archiveParts[0])
  && (appParts[1] === archiveParts[1]);
