...
const { backupProgress, appVersion } = storeToRefs(appStore);
...
// Closing the dialog by any other route - the overlay, ESC - must stop the work
// too, otherwise the service keeps compressing for a window that is gone.
onBeforeUnmount(() => {
  const progress = backupProgress.value;
