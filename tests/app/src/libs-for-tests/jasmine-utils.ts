...
/**
 * jasmine's pending() throws a STRING starting with this marker, so it has to
 * be let through callbackFor untouched — otherwise a deliberate skip comes out
 * of the report as `Failed: => marked Pending…`.
 */
const PENDING_MARKER = '=> marked Pending';

function isPendingSignal(err: unknown): boolean {
	return ((typeof err === 'string') && err.startsWith(PENDING_MARKER));
}

function isConnectException(err: unknown): boolean {
	return ((err as web3n.ConnectException)?.type === 'connect');
}

/**
 * Marks the running spec pending with the given reason, and sends the reason to
 * the run's console as well: the reporter records only the NAME of a pending
 * spec, so the reason needs its own channel to be visible at all.
 */
function skipWithReason(reason: string): never {
	w3n.testStand.log('info', `⏭️  skipped: ${reason}`);
	pending(reason);
	// pending() always throws; this keeps the return type honest.
	throw new Error(reason);
}

/**
 * Runs an action that may never come back, and turns "never came back" into a
 * pending spec instead of a timeout failure. Give the spec a bigger timeout
 * than `millis`, or jasmine cuts the spec off first and the reason is replaced
 * by "did not complete within Nms".
 */
export async function skipSpecIfUnresponsive<T>(
	...
	const timedOut = {};
	try {
		return await callWithTimeout(action, millis, () => timedOut);
	} catch (err) {
		if (err === timedOut) {
			skipWithReason(`${what} did not respond within ${millis} ms`);
		}
		throw err;
	}
}
		...
		if (setup && !setup.isUp) {
			...
			try {
				...
				if (isPendingSignal(err)) {
					// A deliberate skip, not a result - hand it back to jasmine.
					throw err;
				} else if (isConnectException(err)) {
					// The server being unreachable is not a defect of this app.
					// Reporting it as a failure sends the reader after the wrong
					// cause, so it becomes a pending spec with a stated reason.
					skipWithReason(`server is unreachable: ${stringifyErr(err)}`);
