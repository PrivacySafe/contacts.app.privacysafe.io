    ...
    setTimeout(async () => {
      ...
      if (window.preTestProc) {
        try {
          ...
          // Without this the await throws out of the callback, env.execute() is
          // never reached and jasmineDone never fires, so the stand receives NO
          // records at all and reads "tests never started" as a pass. Report the
          // failed setup ourselves so the run fails and names its cause.
          w3n.testStand.record('tests-start');
          w3n.testStand.record('tests-fail',
            'Test setup failed, no spec was executed:\n' +
            (err && err.stack ? err.stack : JSON.stringify(err, null, 2)));
          return;
        }
      }
